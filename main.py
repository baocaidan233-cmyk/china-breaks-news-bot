...
async def run_cycle(
    ...
    # --- Layer 1.5: English-only filter — China Breaks is an English-only
    # channel, same posture as AM1ST; a non-English candidate wastes
    # embedding/scoring cost and, if it slips all the way through, can only
    # be caught downstream by the writer's own judgment (see
    # core/language.py docstring for the real AM1ST incident that showed
    # that alone isn't reliable — this check is ported here as a
    # preventive measure from day one, not added after our own incident).
    # Cheap, on title+description only — the full extracted article gets
    # its own check at publish time in main_publish.py. ---
