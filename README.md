...
2. **抓 RSS**——发布时间在最近 **3 小时**以内（`max_publish_age_hours`，跟 AM1ST 一致，2026-09-06 从原来的 6 小时改回 3 小时，对齐当前真实 n8n 系统的 `filter_feed_hours`）。**没有任何抓取上限**——之前有过单信源条数上限（`rss.max_items_per_feed`）和整批总量上限（`rss.max_total_items`，按发布时间截断）两道限制，都是照抄老 n8n 系统的调参值加的，AM1ST 本身都没有这两道限制，用户也没要求加；2026-09-06 确认总量上限是导致编辑反馈"中国新闻不够"的真实原因之一（纯按新旧截断、不看相关性，源池里大量通用媒体的稿子会把稀少的真中国新闻挤出候选池），用户明确要求"不要自己设置任何上限"，两道限制都已删除，回到跟 AM1ST 一样完全不设上限，交给下面的去重/打分环节自己筛选。
